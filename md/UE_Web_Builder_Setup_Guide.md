...
│       │   └── {Platform}\{Config}\
│       │       ├── Log\             ← 빌드 전체 로그 (build_*.log)
│       │       └── Issue\          ← Warning/Error 필터링 리포트 (issue_*.md)
...
[STEP N/N] Build
      └── BuildProject.bat {platform} {config} [-clean] 실행 (spawn)
              ...
              └── close  → STATUS WebSocket (archivePath / lastError 포함)
                           ├── 로그 파일 저장: Log/build_{timestamp}.log
                           └── Issue 파일 생성: Issue/issue_{timestamp}.md
```
...
빌드가 완료되면 아카이브 경로 하위에 다음 폴더가 자동 생성된다.

```
Saved\Builds\{platform}\{config}\
├── (빌드 결과물 — APK, PAK 등)
├── Log\
│   └── build_{timestamp}.log      ← 빌드 전 과정 전체 로그
└── Issue\
    └── issue_{timestamp}.md       ← Warning/Error 필터링 리포트
```

**Log 파일 (`build_*.log`)**  
빌드 시작부터 종료까지 git 동기화 · 빌드 · Sentry 업로드 모든 단계의 stdout/stderr를 타임스탬프와 함께 기록.

**Issue 파일 (`issue_*.md`)**  
로그 전체에서 Warning/Error 항목만 추출해 중복 제거 후 Markdown 형식으로 저장.
- UE 자체 생성 요약 섹션(`Warning/Error Summary`) 중복 제외
- 빌드 메타 정보 테이블 + Errors 섹션 + Warnings 섹션으로 구성
- 빌드 성공/실패/취소 관계없이 항상 생성
- 터미널 로그에 `[WebBuilder] 🔍 Issue report saved: ...` 로 경로 확인 가능
...
### 11.5 Issue 리포트 자동 생성

빌드 완료 시 로그 파일 저장 직후 Issue 파일을 자동으로 생성한다.

**처리 흐름:**
1. 전체 로그에서 UE 자체 `Warning/Error Summary` 섹션(중복) 제거
2. 나머지 라인 중 `: Warning:` 패턴 → Warnings 목록, `: Error:` 패턴 → Errors 목록으로 분류
3. 타임스탬프·프레임 번호 제거 후 내용 기반 중복 제거 (Set)
4. `Issue/issue_{timestamp}.md` 로 저장

**Issue 파일 포맷 (Markdown):**
```markdown
# Build Issue Report
...
## Warnings (14)
- `LogMaterial: Warning: Failed to compile Material for platform SF_VULKAN_ES31_ANDROID`
- `LogLinker: Warning: Unable to load Paragon_Proto_Retarget ...`
...
```

**저장 경로:**
```
{projectPath}\Saved\Builds\{platform}\{config}\Issue\issue_{timestamp}.md
```

> 빌드 성공/실패/취소 여부와 무관하게 항상 생성됨.  
> Issue 파일이 없으면 `Issue` 폴더도 함께 자동 생성된다.
